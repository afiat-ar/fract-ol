...
/*   Updated: 2021/10/28 10:06:44 by afiat-ar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_start_fract(t_img_data data)
{
	...
	if (!ft_strncmp(data.type, "perpendicular buffalo",
			ft_strlen(data.type) + 1))
		ft_buffalo(data);
...
void	ft_solve_color(t_img_data data, t_fract fract)
{
	...
	if (fract.i == 50)
		...
		my_mlx_pixel_put(&data, fract.x, fract.y,
			fract.i * (color * data.color));
}
